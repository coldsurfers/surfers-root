...
const List = ({sections, thumbnails, ListFooterComponent}: Props) => {
  ...
  }) => ReactElement | null = useCallback(info => {
    ...
    switch (title) {
      ...
      case 'ticket-open-date':
        children = sectionHeaderTitle ? (
          <HeaderItem title={sectionHeaderTitle} />
        ) : null;
        break;
      case 'html':
  ...
  > = useCallback(info => {
    ...
    switch (title) {
      case 'title':
        children = (
          <ConcertTitleItem
            title={(info.item as ConcertDetailTitleSectionData).title}
          />
        );
        break;
      case 'venue':
        children = (
          <GigLocationItem
            location={(info.item as ConcertDetailVenueSectionData).name}
          />
        );
        break;
      case 'lineup':
        children = (
          <LineupItem {...(info.item as ConcertDetailLineupSectionData)} />
        );
        break;
      case 'ticket-seller':
        children = (
          <TicketSellerItem
            {...(info.item as ConcertDetailTicketSellerSectionData)}
          />
        );
        break;
      case 'date':
        children = (
          <GigDateItem
            date={(info.item as ConcertDetailDateSectionData).date}
          />
        );
        break;
      case 'price-info':
        children = (
          <ConcertPriceInfoItem
            priceInfo={{...(info.item as ConcertPriceSectionData)}}
          />
        );
        break;
      case 'ticket-open-date':
        children = (
          <ConcertTicketOpenDateItem
            {...(info.item as ConcertTicketOpenDateSectionData)}
          />
        );
        break;
      case 'html':
        children = (
          <ConcertWebview html={(info.item as ConcertHtmlSectionData).html} />
        );
        break;
      default:
        children = null;
