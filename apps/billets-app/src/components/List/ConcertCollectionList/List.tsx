...
const List = ({ sections, onPressItem, onPressHeaderItem }: Props) => {
  const renderSectionHeader: (info: { section: ConcertCollectionListSection }) => ReactElement = useCallback(
    ({ section }) => {
      const onPress = () => {
        onPressHeaderItem &&
          onPressHeaderItem({
  ...
  const renderItem: SectionListRenderItem<ConcertCollectionListItemT, ConcertCollectionListSectionT> = useCallback(
    (info) => {
