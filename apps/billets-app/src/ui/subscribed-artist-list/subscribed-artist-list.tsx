...
export function SubscribedArtistList({ listHeaderComponent }: { listHeaderComponent?: React.ComponentType<unknown> }) {
  ...
  const { data, fetchNextPage, isFetchingNextPage, isLoading, hasNextPage, isPending, refetch } =
    useSuspenseInfiniteQuery({
      ...
      queryFn: ({ pageParam = 0 }) => apiClient.subscribe.getArtistList({ offset: pageParam, size: PER_PAGE }),
