...
const NativeUpdate = () => {
  ...
  return (
    <SafeAreaProvider>
      <Modal visible>
        <CommonScreenLayout edges={['top']} style={{ width: '100%', justifyContent: 'center', alignItems: 'center' }}>
          <FastImage
          ...
          />
          <Text style={{ fontSize: 16, fontWeight: 'bold', color: semantics.foreground[1], textAlign: 'center' }}>
            {`업데이트가 필요하지만,\n새로운 앱이 출시 되었어요! 🎉`}
          </Text>
          <Text style={{ fontSize: 14, color: semantics.foreground[1], textAlign: 'center', marginVertical: 12 }}>
            {`스토어에서 업데이트를 부탁드릴게요`}
          </Text>
