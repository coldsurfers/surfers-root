...
(
  ...
  select jsonb_build_object(
    ...
    'title', c.title,
    'slug', c.slug,
    'date', to_char(c.date at time zone 'UTC', 'YYYY-MM-DD"T"HH24:MI:SSZ'),
  ...
  where c."deletedAt" is null
    ...
    and (
      ...
      or exists (
        ...
        where cov."concertId" = c.id
          and (
            normalize_text(v."name") ilike '%'||normalize_text(q)||'%'
...
(
  ...
  select jsonb_build_object(
    ...
    'profileImgUrl',
      (
        select "imageURL"
        from "ArtistProfileImage" api
        where api."artistId" = a.id
        order by api."createdAt" asc nulls last
        limit 1
      )
  )
  from "Artist" a
